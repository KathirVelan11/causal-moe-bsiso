...
def train_place_with_gsina(
    ...
    if args.n_samples_cap > 0 and args.n_samples_cap < n_samples_full:
        keep_idx = np.arange(args.n_samples_cap)  # chronological prefix (drift-safe)
    else:
    ...
    encoder = SharedEncoder(in_channels, hidden_channels)
    ...
    for epoch in range(args.epochs):
        ...
        for start in range(0, n_samples - args.batch_size + 1, args.batch_size):
            ...
            for idx in idx_batch:
                ...
                preds = []
                for i in range(env_t.shape[0]):
                    edge_groups = [
                        (gsina_out.causal_edge_index, gsina_out.causal_edge_weight, x_anchor),
                        (gsina_out.spurious_edge_index, gsina_out.spurious_edge_weight, env_t[i]),
                    ]
                    h = encoder(x_anchor, edge_groups, n_clusters)
                    pred = causal_head(h)
                    preds.append(pred)
                    if args.cia_weight > 0.0 and i == 0:
                        # CIA task 2 combined-check only (--cia-weight > 0):
                        # reuse swap-index-0's hidden state -- no extra
                        # forward pass needed, since encoder() was already
                        # being called here for the VREx prediction anyway.
                        cia_hidden_list.append(h[target_idx])
    ...
    # --- Evaluation: identical protocol to step 4/CIA, for comparability --
    generator.eval()
    encoder.eval()
    causal_head.eval()
    expert.eval()
    with torch.no_grad():
        ...
        preds = []
        all_keep_probs = []
        sample_score_idx = rng.choice(n_samples, size=min(200, n_samples), replace=False)
        for idx in eval_idx:
            raw_scores = generator(gen_windows[idx], causal_edge_index_full)
            gsina_out = gsina_split(
                raw_scores, causal_edge_index_full, r=r,
                n_iters=args.gsina_iters, temperature=args.gsina_temperature,
                gumbel_noise=False,  # deterministic at eval
            )
            pred = expert(x_all[idx], target, gsina_out.causal_edge_index, gsina_out.causal_edge_weight)
